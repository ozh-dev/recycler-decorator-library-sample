...
import com.ozh.dsl.decorators.sample.Rules
import com.ozh.dsl.decorators.UNDEFINE_VIEW_HOLDER
import com.ozh.dsl.decorators.Utils.forEach
        ...
        forEach(recyclerView) { view, viewHolder, nextViewHolder ->
            if (viewHolder is DecorableViewHolder) {
                viewHolder.itemDivider?.let { itemDivider ->
                    ...
                    val areSameHolders = viewHolder.itemViewType == nextViewHolder?.itemViewType ?: UNDEFINE_VIEW_HOLDER

                    val drawMiddleDivider = Rules.checkMiddleRule(itemDivider.rule) && areSameHolders
                    val drawEndDivider = Rules.checkEndRule(itemDivider.rule) && areSameHolders.not()
